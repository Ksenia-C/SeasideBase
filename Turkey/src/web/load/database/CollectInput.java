...
public class CollectInput {
    ...
    String getForTitles(String inp) {
    	...
    	for(int i=0;i<inp.length()+1;i++) {
    		if(i == inp.length() || inp.charAt(i) == ';') {
    			...
    			while(j<h.length() && h.charAt(j)!='-') {j++;}
    			if(j==h.length())return printError(h, 0);
    			...
    			for(;j<h.length();j++) {
    				...
    				if(h.charAt(j)!='_' && !is_english(h.charAt(j)) && 
    						!is_int(h.charAt(j))) {
        				return printError(h, j);
    				}
    ...
    String setAct(String title, String date_m, String date_e, int type, String places,
    		String add) {
    	this.title = title;    	this.type = type;
    	this.date_m = date_m;    	this.date_e = date_e;
    	this.places = places;
