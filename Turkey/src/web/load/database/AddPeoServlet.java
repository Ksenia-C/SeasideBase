...
public class AddPeoServlet extends HttpServlet {
	...
	private String[] namePeoParams = {"StudentName", "StudentSurname", "StudentPatronymic",
			"StudentBirthday", "StudentCity"};
	Integer maxid = 1;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	for(int i=0;i<namePeoParams.length;i++) {
    		String value = request.getParameter(namePeoParams[i]);
    		if(value.length()==0) {
    			aboutPeople[i] = null;
    		}else {
    	...
    	try {
        	...
        	// try to organize in sqrt decomposition in finding min elemnt that dousn't meet in sequence
        	...
        	// close connection
        	// just now I don't know if it correctly open and close it every time in one servlet 
        	// maybe more correctly open ones with first request and close after time or with the closing tab
